...
- 参加受付を担当している端末が抜けた場合、残った端末の中から自動で新しい参加受付を選び、同じルームURLとQRのまま継続します。
- 退出ボタンを押さずに閉じた端末は、生存確認が途切れてから参加者一覧から削除されます。
- だれかが参加したときと退出したときには、短い通知音が鳴ります。
...
- 位置情報の利用を許可した端末同士では、Geolocation APIの座標と精度から「明らかに遠い」相手を粗く除外し、その相手には音声相関を回しません。位置情報が取れない場合は従来どおり音声だけで近接判定します。
- マイク取得時にブラウザへ`echoCancellation`、`noiseSuppression`、`autoGainControl`を要求します。実際の効き方は端末、OS、ブラウザ、接続中のイヤホンに依存します。
- マイクOFF中も近接判定用のマイク解析は継続し、WebRTCの送信だけ無音に差し替えます。
- 対応ブラウザではMedia Session API経由でイヤホン側のマイク操作、通話、受話、再生/停止系ボタンを検知し、アプリ内のマイクON/OFFを切り替えます。Bluetooth/HFPの全イベントをWebページから直接取得できるわけではありません。
- PCブラウザでは対応していればアプリ内でスピーカーを選べます。
- AndroidとiOSではBluetoothの入出力は基本的にOS側の音声ルートに従います。
- ポケットロック中は通話継続に必要な音声処理を優先し、参加者一覧や通常通知などの画面更新を解除時まで抑制します。
